...
function createCanvasOnce() {
    const container = document.getElementById("canvas-container");
    if (!container || document.getElementById("myCanvas")) return; // if there IS a canvas, do not redraw
...
// SubtendedAngles Module 
const TAU = 2 * Math.PI;
const RAD2DEG = 180 / Math.PI;

function normalize(a) {
    return (a % TAU + TAU) % TAU; // transforms a confusing negative angle into its positive twin
}

function shortestDiff(start, end) {  // finding the fastest directional route avoiding unnecessary wraps around the circle
    let d = end - start;
